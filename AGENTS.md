...
- **Build an index (the CLI):** `npx wiki-search-index --wiki ./wiki` → `<wiki>/search-index.json`. Add `--file README.md` (repeatable; run from the repo root) to fold repo files in alongside the wiki pages.
...
# Fold a repo file (e.g. README API docs) in alongside the wiki — run from the repo root
npx wiki-search-index --wiki ./wiki --file README.md
```
